...
export function KnowledgeCard({ post }: { post: KnowledgePost }) {
  ...
  return (
    <article
      ...
      className="group flex cursor-pointer gap-3 overflow-hidden rounded-2xl bg-white p-2.5 shadow-sm ring-1 ring-black/5 transition-transform active:scale-[0.99]"
    >
      <img
        src={post.image}
        alt={post.title}
