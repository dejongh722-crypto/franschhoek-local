...
- [x] **Community groups** — public WhatsApp group invite links (admin-managed via `/admin` + scraper-found) shown on the Community page for all members (free discovery). Store `src/store/communityGroups.tsx`, `community_groups` table. Scraper extraction now gap-fills: it runs the Claude pass whenever a source wants a kind (venue/event/deal) that JSON-LD didn't yield, so winery/restaurant pages also surface events & deals.
